 ...
 * home-indicator safe area, and hides itself on /contact (where the calendar
 * already is the action).
 */
export const MobileCtaBar = ({ href = '/contact', label = 'Book a call' }: MobileCtaBarProps) => {
  const reduceMotion = useReducedMotion();
  const onContact = typeof window !== 'undefined' && window.location.pathname === '/contact';
  ...
  return (
    <motion.div
      ...
      className="md:hidden fixed inset-x-0 bottom-0 z-40 px-4 pt-4 pb-[max(14px,env(safe-area-inset-bottom))] bg-gradient-to-t from-paper via-paper/95 to-transparent"
    >
      <a
        href={href}
